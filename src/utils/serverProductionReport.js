...
export const downloadProductionReport = async params => {
...
export const downloadProductionPlanningFile = async ({ id }) => {
  const response = await downloadProductionPlanningFileApi(id);
  const filename = `production-flow-${safeName(id)}.pdf`;
  const path = `${RNFS.DocumentDirectoryPath}/${filename}`;
  const base64 = Buffer.from(response.data).toString('base64');
  await RNFS.writeFile(path, base64, 'base64');
