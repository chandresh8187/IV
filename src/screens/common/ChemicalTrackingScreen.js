...
import {
  getChemicalChecksApi,
  saveChemicalCheckApi,
} from '../../api/chemicalChecksApi';
...
export default function ChemicalTrackingScreen({ navigation }) {
  ...
  const query = useQuery({
    queryKey: ['chemical-checks', month],
    queryFn: () => getChemicalChecksApi({ month }),
    enabled: canView,
    retry: false,
  });
  ...
  const mutation = useMutation({
    ...
    onSuccess: (response, variables) => {
      setSuccess(response.message || 'Chemical check saved.');
      setError('');
      setForm(emptyForm());
    ...
    onError: failure =>
      setError(
        failure?.response?.data?.message || 'Could not save chemical check.',
      ),
  });
  ...
  const save = () => {
    setSuccess('');
    setError('');
    if (!/^\d{4}-\d{2}-\d{2}$/.test(form.inspection_date))
  ...
  const generate = async () => {
    setGenerating(true);
    try {
      const pdf = await downloadChemicalChecksReport(month);
      navigation.navigate('PdfViewer', {
        ...pdf,
        title: `Chemical Checks · ${monthLabel(month)}`,
      });
    } catch (failure) {
      Alert.alert(
  ...
  const selectInspectionDate = (event, selectedDate) => {
    ...
    setForm(current => ({
      ...current,
      inspection_date: formatDate(selectedDate),
    }));
    setError('');
  };
  if (!canView)
    return (
      <View style={styles.center}>
        <Text style={styles.muted}>
          You do not have chemical tracking access.
        </Text>
      </View>
    );
  ...
  return (
    ...
    >
      ...
      >
        <View>
          <Text style={styles.title}>Chemical Tracking</Text>
          <Text style={styles.muted}>
            Daily flux temperature and flux and acid pH and density checks
          </Text>
        </View>
          ...
          <View style={styles.card}>
            <View>
              ...
              <TouchableOpacity
                ...
                accessibilityLabel="Select inspection date"
            ...
            {showDatePicker && (
              <View style={styles.datePickerWrap}>
                <DateTimePicker
                  value={parseDate(form.inspection_date)}
                  mode="date"
                  display={Platform.OS === 'ios' ? 'spinner' : 'default'}
                  maximumDate={new Date()}
                  onChange={selectInspectionDate}
                />
                {Platform.OS === 'ios' && (
                  <TouchableOpacity
            ...
            >
              {mutation.isPending ? (
                ...
                <Text style={styles.buttonText}>Save chemical check</Text>
              )}
            </TouchableOpacity>
          </View>
        )}
        ...
        {query.isLoading ? (
          ...
          <TouchableOpacity style={styles.card} onPress={refetch}>
            <Text style={styles.error}>
              {query.error?.response?.data?.message ||
                'Could not load chemical checks. Tap to retry.'}
            </Text>
          </TouchableOpacity>
        ) : rows.length ? (
          rows.map(item => (
            <View style={styles.card} key={item.id}>
              ...
              <View style={styles.readingGrid}>
                <Reading label="Flux pH" value={reading(item.flux_ph)} />
                <Reading
                  label="Flux density"
                  value={`${reading(item.flux_density)} g/cm³`}
                />
                <Reading
                  label="Flux temperature"
                  value={
                    item.flux_temperature_c == null
                      ? '—'
                      : `${reading(item.flux_temperature_c)} °C`
                  }
                />
                <Reading label="Acid pH" value={reading(item.acid_ph)} />
                <Reading
                  label="Acid density"
                  value={`${reading(item.acid_density)} g/cm³`}
                />
              </View>
          ...
          <View style={styles.card}>
            <Text style={styles.muted}>
              No chemical checks recorded for this month.
            </Text>
          </View>
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  dateHint: { color: COLORS.muted, fontSize: 11, marginTop: 3 },
  datePickerWrap: {
    backgroundColor: COLORS.surfaceMuted,
    borderRadius: UI.radiusSmall,
    padding: 8,
  ...
  entryDate: { color: COLORS.text, fontSize: 16, fontWeight: '800' },
  checkedBy: { color: COLORS.primary, fontWeight: '700', fontSize: 13 },
  ...
  readingLabel: { color: COLORS.muted, fontSize: 11 },
  readingValue: {
    color: COLORS.text,
    fontSize: 16,
    fontWeight: '800',
