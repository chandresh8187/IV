...
export default function ZincStockScreen({ navigation }) {
  ...
  const movement = useMutation({ mutationFn: saveZincMovementApi, retry: false,
    onSuccess: response => { Keyboard.dismiss(); client.setQueryData(['zinc-stock'], response); pendingRequest.current = null; setAction(null); setAmount(''); setReceiptRate(''); setNote(''); setError(''); setSuccess(response.message); refresh(); },
  ...
  const saveMovement = () => {
    ...
    const rate = Number(receiptRate);
    if (action === 'receive' && (!/^\d+(\.\d{1,2})?$/.test(receiptRate) || !Number.isFinite(rate) || rate <= 0)) return setError('Enter the received zinc rate per kg with up to 2 decimal places.');
    if (action === 'transfer' && preview?.error) return setError(preview.error);
    const payload = { action, amount_kg: kg, ...(action === 'receive' ? { zinc_rate_per_kg: rate } : {}), note: note.trim() }; const signature = JSON.stringify(payload);
